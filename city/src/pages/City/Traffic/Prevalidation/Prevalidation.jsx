...
//URLS
import { URL_MODIFICAR_ALERTAS, URL_OBTENER_PREVALIDACION_ALERTAS, URL_VALIDAR_ALERTA } from '../../../../../api/connections/urls'
...
import { direccionCamara as autocompleteDireccion, tiposVehiculo as autocompleteTiposVehiculo, url_path } from '../../../../../constants/common'
...
export function Prevalidation() {
    ...
    //Context
    const { module, setInfoMessage, setIsLoading, filterTrafficAlert, setFilterTrafficAlert, filterAlertCode, setFilterAlertCode, currentDevice, setCurrentDevice, filterInfractionCode, setFilterInfractionCode, requestAPI, url_origin, autocompleteCountries } = useContext(MainDataContext)
    ...
    const [dataFiltered, setDataFiltered] = useState(filterAlertCode || filterTrafficAlert || filterInfractionCode ? true : false)
    ...
    //Elementos filtrar
    const [filterState, setFilterState] = useState(filterInfractionCode ? {alerts: filterInfractionCode} : {})
    ...
    const getPrevalidations = async (payload) => {
        ...
        const params = {
            ...
            fecha_ini: filterInfractionCode ? moment().format('YYYY-MM-DD') : payload?.initial_date,
            fecha_fin: filterInfractionCode ? moment().format('YYYY-MM-DD') : payload?.final_date,
            hora_ini: payload?.initial_time,
            hora_fin: payload?.final_time,
            cod_dispositivo: currentDevice?.cod_dispositivo ?? payload?.device?.cod,
            alertas: filterInfractionCode ?? (payload === 'reset' ? null : payload?.alerts),
        ...
        //Si no estamos filtrando, le añadimos un limite de 200 resultados
        if (!isFiltering) { 
            resetFilter()
            params.limit = 200 
        }
        ...
        setData(data)
        setSortedData(data)
        isFiltering ? setSubtitle(t('messages.resultsFiltered', { value: data?.length?.toLocaleString(i18n.language) })) : setSubtitle(t('messages.resultsLast', { value: data?.length?.toLocaleString(i18n.language) }))
    ...
    const editPrevalidation = async (payload) => {
        ...
        //Función que para filtrar reconocimientos.
        let response = await requestAPI(URL_MODIFICAR_ALERTAS, params, 'city')
    ...
    */
    ...
    useEffect(() => {
        ...
        if (filterAlertCode === null && filterTrafficAlert === null && filterInfractionCode === null && currentDevice === null) {
            getPrevalidations('reset')
        }
    ...
    //Si recargamos la sección mediante un click a una alerta, que realice la función y la pantalla no se quede colgada
    useEffect(() => {
        if (filterAlertCode !== null || filterTrafficAlert !== null || filterInfractionCode !== null || currentDevice !== null) {
            setCurrentData(undefined)
            getPrevalidations()
            ...
            setFilterInfractionCode(null)
            setCurrentDevice(null)
        }
        //eslint-disable-next-line
    }, [filterAlertCode, filterTrafficAlert])

    //Actualizamos el detailModal si hemos modificado una prevalidación
    useEffect(() => {
        currentData !== undefined && setCurrentData(sortedData.filter(id => id?.cod_alerta === currentData.cod_alerta)[0])
    ...
    return <>
        ...
        {/*Filtro*/}
        {filterOpen &&
            <FilterSection title={t('crud.filterElements', { elements: t('terms.alerts') })} setIsOpen={setFilterOpen} onReset={() => resetFilter()} onSubmit={getPrevalidations} rows={6} columns={4} onChange={setFilterState} state={filterState} unequalRows customStyles={{ width: '70dvw', maxWidth: '1250px' }} submitText={t('buttons.filter')} submitIcon={filterIcon}>
        ...
        {currentData &&
            <DetailModal
                ...
                consultingSection={'traffic-consulting-consults'}
        ...
        {/*Página Prevalidación de Traffic*/}
                ...
                <Box routes={[{ name: t('sections.PREVALIDACION') }]}>
                    ...
                    {listStyle ?
                        <Table
                            ...
                            columnStyles={['element--short', 'element--short', 'element--short', 'element--medium', 'element--short', 'element--long', 'element--short']}
                            row_elements={[
                                ...
                                {(item, index) =>
                                    <GridReport
                                        ...
                                        data={{
                                            ...
                                            description: parseInt(item?.cod_alertagest) === 1 ? `${t('codes.cityAlerts.' + item.cod_alertagest)}: ${item.velocidad_vehiculo}km/h - ${item.velocidad_max}km/h` : `${t('codes.cityAlerts.' + item.cod_alertagest)}`,
