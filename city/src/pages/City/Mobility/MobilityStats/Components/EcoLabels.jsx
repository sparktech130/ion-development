...
export const EcoLabels = ({
    ...
    //obtiene pedidos
    const getData = async () => {
        setLoadingsControl(prev => ({ ...(prev || {}), 'ecoLabels': true }))
        try {
            ...
            if (!data.error) {
                ...
                sortedData.forEach(item => {
                    if (item.distintivo?.trim()) {
                        labels.push(t('titles.ecoLabel', { label: item.distintivo }))
            ...
            setTimeout(() => {
                setLoadingsControl(prev => ({ ...(prev || {}), 'ecoLabels': false }))
            }, 300);
        }
    }
        ...
        <>
            ...
            >
                <HorizontalBarChart
                    labels={labels}
                    datos={[datos]}
                    colors={["#6692DB", "#4CD984", "#BEF0D2", "#FFE37E", "#E72584"]}
