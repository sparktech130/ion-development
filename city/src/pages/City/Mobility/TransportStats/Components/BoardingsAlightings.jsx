...
//Assets
import boardingsIcon from "@icons/actions/boardings.svg?react"
import alightingsIcon from "@icons/actions/alightings.svg?react"


export const BoardingsAlightings = ({
    ...
    //Graph
    const [datos, setDatos] = useState({ labels: [], datos: [[], []], boardings: 0, alightings: 0 })
    ...
    const getData = async () => {
        ...
        let error = true
        setLoadingsControl(prev => ({ ...(prev || {}), 'ba': true }))
        ...
        try {
            ...
            if (!data.error && Array.isArray(data)) {
                ...
                data?.forEach(item => {
                    ...
                    //implementar cuando sepa cómo vienen los datos (separar en boarding y alightings) y calcular totales
                    datos[0].push(0)
                    datos[1].push(0)
                })
                ...
                //asignar datos
                setDatos({ labels: labels, datos: datos, total: total, boardings: boardings, alightings: alightings })
            ...
            setTimeout(() => {
                if (error) {
                    setDatos({ labels: [], datos: [[], []], boardings: 0, alightings: 0 })
                }
                setLoadingsControl(prev => ({ ...(prev || {}), 'ba': false }))
            }, 300);
        }
    }
    ...
    return (
        <>
            ...
            <StatsCard
                title={t('titles.boardingsAndAlightings')}
                description={[(datos?.boardings || 0) + ' ' + t('terms.boardings'), (datos?.alightings || 0) + ' ' + t('terms.alightings')]}
                descriptionIcon={[boardingsIcon, alightingsIcon]}
            ...
            >
                <BarChartV4
                    ...
                    etiquetas={[t('params.boardings'), t('params.alightings')]}
