...
//Assets
import speedIcon from '@icons/alerts/speeding.svg?react'
...
export const Speed = ({
    ...
    const getData = async () => {
        ...
        setLoadingsControl(prev => ({ ...(prev || {}), 'speed': true }))
        ...
        try {
            ...
            let params = {
                campos: groupByDay ? ['fecha', 'velocidad'] : ['fecha', 'hora', 'velocidad'],
            ...
            if (data && !data.error) {
                ...
                //dar formato datos
                data?.forEach(item => {
                    if (item.velocidad_media) {
                    if (groupByDay) {
                        labels.push(item.fecha)
                    } else {
                        labels.push(item.fecha+ '\n' + item.hora + ':00')
                    }
                        datos.push(parseFloat(item.velocidad_media)?.toFixed(2) || 0)
                    }
                });
                ...
                setMedia(calcularVelocidadMediaTotal(data))
            ...
            setTimeout(() => {
                setLoadingsControl(prev => ({ ...(prev || {}), 'speed': false }))
            }, 300);
        }
    }
    ...
    //Calcula la media total entre varias medias. Hay que tener en cuenta cuantos registros se ha tenido en cuenta para calcular la media de cada hora. Si no no es la media real total (para una velocidad_media de una hora puede haber 1 registro y para otra 1000)
    function calcularVelocidadMediaTotal(datos) {
        if (!checkArray(datos)) return 0;

        let sumaPonderada = 0;
        let totalGlobal = 0;

        for (const item of datos) {
            const velocidad = parseFloat(item.velocidad_media);
            const total = parseInt(item.total);

            if (!isNaN(velocidad) && !isNaN(total) && total > 0) {
                sumaPonderada += velocidad * total; //total es el número de registros que se han tenido en cuenta para calcular velocidad_media de esa hora
                totalGlobal += total;
            }
        }

        return totalGlobal > 0 ? (sumaPonderada / totalGlobal)?.toFixed(0) : 0;
    }
    ...
    return (
        <>
            ...
            <StatsCard
                title={t('params.averageSpeed')}
                description={[t('messages.thereAreNoSpeedExcesses')]}
                descriptionIcon={[speedIcon]}
                value={(media?.toLocaleString(i18n.language) || 0) + ' Km/h'}
