...
//Constants
import { autocompleteColors, alert_codes_traffic } from '../../../../../constants/common';
...
export function Consulting() {
    ...
    const handleSaveCSV = () => {
        const registros = sortedData.filter(x => x.checked)
    ...
    const csvExport = {
        data: sortedData.filter(value => value.checked === true),
        headers: [
    ...
    return <>
        ...
        {/*Filtro*/}
        {filterOpen &&
            <FilterSection title={t('crud.filterElements', { elements: t('terms.recognitions') })} setIsOpen={setFilterOpen} onReset={() => resetFilter()} onSubmit={getRecognitions} rows={6} columns={4} onChange={setFilterState} state={filterState} unequalRows submitText={t('buttons.filter')} customStyles={{ width: '70dvw', maxWidth: '1250px' }} submitIcon={filterIcon}>
                    ...
                    {listStyle ?
                        <Table
                            ...
                            checkRedRowFunction={(item) => {
                                ...
                                const hasTrafficAlert = alerts?.some(alert => alert_codes_traffic?.includes(alert));
                                return hasTrafficAlert;
                            }}
                        />
                        :
